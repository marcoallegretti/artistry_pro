...
class AnimationService {
  ...
  AnimationFrame addNewFrame() {
    ...
    final layersCopy = _frames[_currentFrameIndex].layers.map((layer) => layer.copyWith()).toList();
  ...
  void moveFrame(int oldIndex, int newIndex) {
    if (oldIndex < 0 || oldIndex >= _frames.length || 
        newIndex < 0 || newIndex >= _frames.length) {
      return;
    }
    ...
    } else if (_currentFrameIndex > oldIndex && _currentFrameIndex <= newIndex) {
      _currentFrameIndex--;
    } else if (_currentFrameIndex < oldIndex && _currentFrameIndex >= newIndex) {
      _currentFrameIndex++;
    }
  ...
  List<MapEntry<AnimationFrame, double>> getOnionSkinFrames() {
    ...
    for (int i = 1; i <= _settings.onionSkinningBefore; i++) {
      ...
      if (frameIndex >= 0) {
        ...
        final opacity = _settings.onionSkinningOpacity * (1 - (i - 1) / _settings.onionSkinningBefore);
    ...
    for (int i = 1; i <= _settings.onionSkinningAfter; i++) {
      ...
      if (frameIndex < _frames.length) {
        // Opacity decreases with distance from current frame
        final opacity = _settings.onionSkinningOpacity * (1 - (i - 1) / _settings.onionSkinningAfter);
        result.add(MapEntry(_frames[frameIndex], opacity));
      }
    }
