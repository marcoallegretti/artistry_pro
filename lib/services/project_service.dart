...
class ProjectService extends ChangeNotifier {
  ...
  Future<void> saveCurrentProject({
    ...
    if (canvasKey != null) {
      ...
      thumbnailPath =
          await _generateAndSaveThumbnail(canvasKey, _currentProject!.id);
    } else {
    ...
    final projectIndex =
        _projects.indexWhere((p) => p.id == _currentProject!.id);
    if (projectIndex >= 0) {
      ...
      if (projectsListJson != null) {
        ...
        _projects = projectsList.map((json) {
          return Project(
            ...
            canvasSettings:
                CanvasSettings.defaultSettings, // Just placeholder for list
    ...
    try {
      ...
      final List<Map<String, dynamic>> layersData = project.layers.map((layer) {
        ...
        if (layer.contentType == ContentType.drawing) {
          ...
          debugPrint(
              'Serializing layer ${layer.name} with ${points.length} points');
          ...
          for (var i = 0; i < points.length; i++) {
            ...
            } else if (point is DrawingPoint) { // From canvas_screen.dart
      ...
      debugPrint(
          'Project data saved successfully: $success (key: $projectKey)');
      ...
      if (projectJson != null) {
        ...
        final layers = layersJson.map<Layer>((layerJson) {
          ...
          if (contentType == ContentType.drawing) {
            ...
            debugPrint(
                'Deserializing layer points, found ${pointsJson?.length ?? 0} points');
            ...
            if (pointsJson != null && pointsJson.isNotEmpty) {
                ...
                if (pointJson == null) {
                  ...
                  try {
                    final Paint paint = Paint()
                      ...
                      ..blendMode = ui.BlendMode.values[pointJson['blendMode']];

                    deserializedPoints.add(DrawingPoint( // From canvas_screen.dart
                      Offset(
                          pointJson['x'].toDouble(), pointJson['y'].toDouble()),
                      paint,
              ...
              debugPrint(
                  'Successfully deserialized ${deserializedPoints.length} drawing points');
            } else {
              payload = <DrawingPoint?>[];  // DrawingPoint from canvas_screen.dart
          ...
          return Layer(
            ...
            blendMode: ui.BlendMode.values[layerJson['blendMode']],
  ...
  Future<String?> _generateAndSaveThumbnail(
      GlobalKey canvasKey, String projectId) async {
    try {
      ...
      final image = await boundary.toImage(
          pixelRatio: 0.3); // Very small thumbnail for storage
